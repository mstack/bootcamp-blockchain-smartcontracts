...
function generateFilesForContract(contracts, contractFilename) {
    ...
    const namespace = preferredNamespace.endsWith('***') ? preferredNamespace.replace('***', contractName) : preferredNamespace;

    generateContractService(outputPath, contractName, namespace, abi, code, 'cs-service');
}

if (combineContracts) {
    const combinedName = `${combinedContractName}-combined.sol`;
    console.log(`Generate combined contract file '${combinedName}'`);
    fs.writeFileSync(path.join(outputPath, combinedName), `${combinedContractContent}\r\n}`, 'utf-8');
}
